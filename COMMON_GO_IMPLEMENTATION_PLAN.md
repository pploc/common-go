...
`common-go` targets Go 1.26 and pins published `github.com/pploc/proto-go v1.1.0` without a `replace` directive. No `common-go` release tag exists.
...
The reusable Kafka contract workflow provisions Kafka and Schema Registry 7.7.1, validates `gym-proto v1.1.0` fixtures, runs live tests, and uploads sanitized evidence. CI and tag validation invoke that workflow after source verification.
